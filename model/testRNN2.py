...
class SequenceClassification:
    ...
    def prediction(self):
        ...
        for i in range(self.num_layers):
            cell = tf.nn.rnn_cell.BasicLSTMCell(num_units=self._num_hidden, state_is_tuple=True, forget_bias=1.0)
            cell = tf.nn.rnn_cell.DropoutWrapper(cell, output_keep_prob=1.0-self.dropout[i])
            stacked_rnn.append(cell)            
        network = tf.nn.rnn_cell.MultiRNNCell(cells=stacked_rnn, state_is_tuple=True)
        ...
        print("target",self.target.get_shape()[1])
    ...
    def optimize(self):
        ...
        optimizer = tf.train.RMSPropOptimizer(learning_rate=0.0003, momentum=0.9, epsilon=1e-6)        
...
def returnTrainDevTestData():
    ...
    matrices = load_data('RNN/smallSample.npy')
    keys = pickle.load(open('RNN/smallSample_keys.pkl', 'rb'))        
    ...
    for idx in range(len(keys)):
        ...
        if m.shape[0] < 128:
          continue 
        # 分成 train,dev and test
        if r < .8:
          train_data.append(m)
          train_labels.append(dr)
        # Dev set helps us in reducing the complexity of diagnosis if things won’t go fine
        elif r < .9:
          dev_data.append(m)
...
if __name__ == '__main__':
    ...
    dropouts_dev  = [0.4, 0.6] #dropout values for different network layers [for test and dev]
    ...
    for step in range(training_steps):
        ...
        if epoch > bestEpoch or epoch == 0:
            ...
            if epoch > 5 and acc > maxTestAccuracy:
                maxTestAccuracy = acc
                bestEpoch = epoch
                # save_path = saver.save(sess, 'RNNmodels/bestRNN/')
                # print('Model saved in path: {}, Accuracy: {:.2f}%, Epoch: {:d}'.format(save_path, 100*acc, epoch))
                print('Model saved  Accuracy: {:.2f}%, Epoch: {:d}'.format( 100*acc, epoch))
    ...
    sess = tf.Session()
    sess.run(tf.global_variables_initializer())
    # saver.restore(sess, 'RNNmodels/bestRNN/')
